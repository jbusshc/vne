...
// cualquier config) pero SOLO tienen una definicion real cuando SZ_EDITOR esta definido
// (Dev). Los llamantes deben envolver sus usos en `#if defined(SZ_EDITOR)` de todas
...
struct EditorDiagnostics {
    ...
    // Textura del atlas de sprites, para el visor de M15 (criterio de SPEC.md #12: "el
    // visor de atlas muestra la textura real, no un recuento de sprites"). El editor no la
    // posee ni la carga: la dibuja.
...
// SZ_EDITOR", despues de render_flush() y antes de render_present()). script_path es el .vns
