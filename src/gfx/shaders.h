...
struct GfxTransitionUniforms {
    f32 threshold = 0.0f;
    f32 sharpness = 1.0f;
    f32 _pad[2]   = {};
};
static_assert(sizeof(GfxTransitionUniforms) == 16);

inline sg_shader_desc gfx_transition_shader_desc(sg_backend backend) {
    sg_shader_desc desc{};
    ...
    desc.uniform_blocks[0].size              = sizeof(GfxTransitionUniforms);
