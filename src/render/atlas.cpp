...
bool atlas_load(Arena* arena) {
    ...
    if (!pak_resolve_into_arena("atlas_00.bin", arena, &bytes, &size)) {
        log_error("atlas_load: no se encontro 'atlas_00.bin'; ejecuta sz_bake primero");
        return false;
    }
    ...
    if (header[1] != k_atlas_bin_version) {
        ...
        // sz_bake, igual que el .vnc desde el .vns. Solo los datos del jugador
...
bool atlas_find(const char* logical_name, AtlasSprite* out) {
    ...
    // Biseccion sobre la tabla ordenada por hash (la ordena sz_bake al escribirla).
    u32 lo = 0, hi = g_count;
    while (lo < hi) {
        u32 mid = lo + (hi - lo) / 2;
...
const char* atlas_name_at(u32 index) {
    if (g_entries == nullptr || g_name_pool == nullptr || index >= g_count) {
        return "";
    }
    return g_name_pool + g_entries[index].name_offset;
}
