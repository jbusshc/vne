...
void* render_alloc_counted(size_t size, void* /*user*/) {
    return heap_guard_malloc(size, HeapSource::Sokol);
}

void render_free_counted(void* p, void* /*user*/) {
    heap_guard_free(p);
}
...
u32 g_render_draw_call_count = 0;
u32 g_render_sprite_count_last_frame = 0;
u32 g_render_frame_index             = 0;
...
// en render_begin_frame y hay que volver a pedirla cada frame.
bool              g_transition_active   = false;
RenderTransitionMask g_transition_mask     = RenderTransitionMask::Fade;
...
// funcion del motor). Ver render/shaders.h para que significa cada una en la formula.
...
TextureHandle make_transition_mask(RenderTransitionMask kind) {
    i32 w = 1, h = 1;
    switch (kind) {
        case RenderTransitionMask::Fade:     w = 1;   h = 1;  break;
        case RenderTransitionMask::Wipe:     w = 512; h = 1;  break;
        case RenderTransitionMask::Dissolve: w = 64;  h = 64; break;
    }
...
// Las tres se crean en render_init, NO perezosamente en el primer uso: crearlas dentro de
// render_flush significaria llamar a sg_make_image/sg_update_image con una render pass
// abierta, que sokol_gfx no permite (el juego moria nada mas arrancar la primera vez que
// se escribio asi).
TextureHandle transition_mask_texture(RenderTransitionMask kind) {
...
f32 transition_sharpness(RenderTransitionMask kind) {
    switch (kind) {
        case RenderTransitionMask::Fade:     return 1.0f;
        case RenderTransitionMask::Wipe:     return 24.0f;
        case RenderTransitionMask::Dissolve: return 24.0f;
    }
    return 1.0f;
}
...
bool render_init(PlatformWindow* window) {
    g_window = window;
    ...
    sg_desc desc{};
    desc.environment = rhi_environment();
    ...
    desc.allocator.alloc_fn = render_alloc_counted;
    desc.allocator.free_fn  = render_free_counted;
    sg_setup(&desc);
    if (!sg_isvalid()) {
    ...
    sg_shader_desc shd_desc = rhi_sprite_shader_desc(sg_query_backend());
    ...
    // shader (mascara + umbral, ver render/shaders.h), asi que se parte del mismo desc.
    sg_shader_desc trans_shd_desc = rhi_transition_shader_desc(sg_query_backend());
    ...
    // (que es donde estaria si se crearan perezosamente desde render_flush).
    for (u32 i = 0; i < 3; ++i) {
        g_transition_mask_textures[i] = make_transition_mask(static_cast<RenderTransitionMask>(i));
    }
...
void render_begin_frame() {
    ...
    g_render_sprite_count_last_frame = g_sprite_count;  // lo encolado en el frame anterior
    g_render_frame_index += 1;
    g_sprite_count     = 0;
    g_render_draw_call_count = 0;
...
void render_draw_sprite(const Sprite& s) {
    if (g_sprite_count >= k_max_sprites_per_frame) {
        log_error("render_draw_sprite: cola llena (%u), se descarta el sprite", k_max_sprites_per_frame);
        return;
    }
...
void render_flush() {
    ...
    // que el blit de letterbox de render_present: sg_append_buffer no se llama dentro de una
...
void render_window_to_virtual(i32 window_w, i32 window_h, f32 window_x, f32 window_y,
    ...
    Recti vp = render_letterbox_rect(window_w, window_h, k_virtual_width, k_virtual_height);
    if (vp.w <= 0 || vp.h <= 0) {
        // Ventana degenerada (minimizada): no hay punto virtual que devolver. Se manda
        // fuera de la pantalla virtual, que es lo mismo que decir "el raton no esta sobre
        // nada".
        *out_virtual_x = -1.0f;
        *out_virtual_y = -1.0f;
        return;
    }
    *out_virtual_x = (window_x - static_cast<f32>(vp.x)) *
                     (static_cast<f32>(k_virtual_width) / static_cast<f32>(vp.w));
    *out_virtual_y = (window_y - static_cast<f32>(vp.y)) *
                     (static_cast<f32>(k_virtual_height) / static_cast<f32>(vp.h));
}

void render_present(i32 window_w, i32 window_h) {
    sg_swapchain sc = rhi_begin_frame(window_w, window_h);
    Recti        vp = render_letterbox_rect(window_w, window_h, k_virtual_width, k_virtual_height);
    ...
    // SPEC.md #6.5: "editor_render() -> solo si SZ_EDITOR", dentro de la misma pasada al
...
bool render_capture_thumbnail(u8* out_rgb, i32 out_w, i32 out_h) {
    return rhi_capture_thumbnail(g_scene_color_image, out_rgb, out_w, out_h);
}
